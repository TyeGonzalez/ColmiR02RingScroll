...
def circular_median(angles, resolution=600):
    """
...
async def handle_notification(sender, data):
    ...
    previous_filtered_angle = df["filtered_angle"].values[-4]
        ...
        if time.time() - last_scroll_time > DEBOUNCE_INTERVAL:
            ...
            if abs(angle_diff) > 0.1:
                if abs(angle_diff) < 2:  
                    ...
                    scroll_amount = (SCROLL_SCALE * angle_diff)
                    # scroll the screen
                    print(f"Scrolling by {scroll_amount} pixels, (angle_diff: {angle_diff})")
